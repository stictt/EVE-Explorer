...
#pragma warning disable CS4014 
    ...
    public class RequestApiScheduler<T>
    {
        ...
        private ILoggerFactoryBase _iLoggerFactory;

        private int _maxRetryCount = 5;
        private int _retryCount = 0;
        private bool IsError = false;

        public RequestApiScheduler(ILoggerFactoryBase loggerFactory, BuilderRequestScheduler builderRequestScheduler)
        {
        ...
        public async Task<List<T>> Start()
        {
            ...
            while (!_tokenSource.IsCancellationRequested)
            {
                ...
                { 
                    ...
                    _logger.LogWarning(string.Format("Request operation failed - {0:f}",DateTime.Now));
                    break;
                }
                CheckingСompleted();
            }
            _logger.LogInformation(string.Format("Result count orders page {0}", _resultRequests.Count));
            return GetResult();
        }
        ...
        private async Task<BaseResponseHttp> ProcessResponseAsync(Task<BaseResponseHttp> responseHttp)
        {
            ...
            {
                _logger.LogError(string.Format("Unexpected Exception {0}:{1:f}", e.GetType().ToString(), DateTime.Now));
