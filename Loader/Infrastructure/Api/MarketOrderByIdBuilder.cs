...
{
    public class MarketOrderByIdBuilder : BuilderRequestScheduler
    {
        private static string Api = @"https://esi.evetech.net/latest/markets/{0}/orders/?datasource=tranquility&order_type=all&type_id={1}&page={2}";
        private List<int> _marketOrders = new List<int>();
        private List<int> _marketTypesId = new List<int>();
        ...
        public MarketOrderByIdBuilder(int parallelCount, List<int> regionsId, List<int> typeId, ILoggerFactoryBase loggerFactory)
        {
        ...
        public override List<IPlanRequest> GetPlanRequests()
        {
            ...
            {
                _marketTypesId.ForEach(xx =>
                {
                    result.Add(new PlanRequestMarketIdApi()
                    {
                        Page = 1,
                        PatternURL = Api,
                        TypeId = xx,
        ...
        public override List<IPlanRequest> UpdateQueryPlan(BaseResponseHttp responseHttp)
        {
            ...
            _logger.LogInformation(String.Format("Count page Plan {0}", page));
